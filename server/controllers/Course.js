...
// Get all students' progress for a course (for admin/instructor)
exports.getAllStudentsProgressForCourse = async (req, res) => {
  try {
    const { courseId } = req.body;
    // Find the course and populate studentsEnrolled
    const course = await Course.findById(courseId).populate('studentsEnrolled');
    if (!course) {
      return res.status(404).json({ success: false, message: 'Course not found' });
    }
    // Get all course content and total number of videos
    let totalVideos = 0;
    ...
    const studentsProgress = await Promise.all(
      course.studentsEnrolled.map(async (student) => {
        const progressDoc = await require('../models/CourseProgress').findOne({
          courseID: courseId,
          userId: student._id,
        });
        const completed = progressDoc?.completedVideos.length || 0;
        const progressPercentage = totalVideos === 0 ? 100 : Math.round((completed / totalVideos) * 10000) / 100;
        return {
    ...
    return res.status(200).json({ success: true, data: studentsProgress });
  } catch (error) {
